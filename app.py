...
def _apply_preset(prefix: str):
    """When preset changes, update start/end date inputs in session_state."""
    preset = st.session_state.get(f"{prefix}_preset", "직접선택")
    if preset == "직접선택":
        return
    s_str, e_str = preset_range(preset)
    st.session_state[f"{prefix}_d1"] = datetime.strptime(s_str[:10], "%Y-%m-%d").date()
    st.session_state[f"{prefix}_d2"] = datetime.strptime(e_str[:10], "%Y-%m-%d").date()

def _mark_custom(prefix: str):
    """If user edits dates manually, mark preset as custom."""
    if st.session_state.get(f"{prefix}_preset") != "직접선택":
        st.session_state[f"{prefix}_preset"] = "직접선택"

def _ensure_range_state(prefix: str, default_preset: str):
    """Initialize preset/start/end keys once so widgets stay in sync."""
    if f"{prefix}_preset" not in st.session_state:
        st.session_state[f"{prefix}_preset"] = default_preset
    if f"{prefix}_d1" not in st.session_state or f"{prefix}_d2" not in st.session_state:
...
def run_all(platform, d_f, d_t, tabula_file=None):
    try:
        ...
        fname = f"통합리포트_{datetime.now().strftime('%m%d_%H%M')}.csv"
...
def run_keyword_report(platform, d1, d2):
    try:
        ...
        fname_xlsx = f"키워드성과_가공본_{datetime.now().strftime('%m%d_%H%M')}.xlsx"
        fname_csv  = f"키워드성과_가공본_{datetime.now().strftime('%m%d_%H%M')}.csv"
        df_out.to_excel(fname_xlsx, index=False)
        df_out.to_csv(fname_csv, index=False, encoding="utf-8-sig")  # 한글/엑셀 호환
        ...
        return summary, detail, (fname_xlsx, fname_csv)
...
def generate_daily_comment_from_file(excel_path: str, platform: str, compare_mode: str, manual_actions: str = "") -> str:
    # excel_path는 과거 호환을 위해 변수명 유지(실제로는 csv/xlsx 둘 다 가능)
...
<style>
/* --- App chrome --- */
:root { --brand: #2563eb; --brand2:#60a5fa; --ink:#0f172a; }
...
.hero h1 { font-size: 1.8rem; margin: 0; font-weight: 800; letter-spacing: -0.02em; }
...
div[data-baseweb="radio"] label { border-radius: 999px !important; }
...
if "daily_file_bytes" not in st.session_state: st.session_state.daily_file_bytes = None
if "daily_file_name"  not in st.session_state: st.session_state.daily_file_name  = None
if "kw_file_bytes"    not in st.session_state: st.session_state.kw_file_bytes    = None
if "kw_file_name"     not in st.session_state: st.session_state.kw_file_name     = None
...
with col_daily:
    ...
    # ✅ 기간/날짜 위젯 동기화 (preset 변경 → 시작/종료 자동 반영)
    _ensure_range_state("daily", default_preset="어제")
    ...
    st.selectbox(
        "기간",
        ["주간(월~일)", "어제", "지난 7일", "지난 30일", "이번 달", "직접선택"],
        key="daily_preset",
        on_change=lambda: _apply_preset("daily"),
    )

    col_d1, col_d2 = st.columns(2)
    with col_d1:
        d1 = st.date_input("시작일", key="daily_d1", on_change=lambda: _mark_custom("daily"))
    with col_d2:
        d2 = st.date_input("종료일", key="daily_d2", on_change=lambda: _mark_custom("daily"))

    tabula_file = st.file_uploader("📎 타뷸라 raw 파일 업로드 (선택, CSV or XLSX)",
                                   type=["csv","xlsx"], key="tabula_upload")

    if st.button("통합 리포트 생성 (CSV)", type="primary", key="btn_daily"):
        # 타뷸라 파일 임시 저장
        tabula_path = None
        if tabula_file:
            import tempfile
        ...
        if fname and os.path.exists(fname):
            ...
            # ✅ 다운로드 성능 개선: 파일을 미리 bytes로 읽어 세션에 저장
            with open(fname, "rb") as f:
                st.session_state.daily_file_bytes = f.read()
            st.session_state.daily_file_name = os.path.basename(fname)

            st.success("✅ 통합 리포트 생성 (CSV) 완료!")

# ✅ 생성된 파일이 있으면 언제든 다운로드 가능 (재생성/재읽기 최소화)
    if st.session_state.get("daily_file_bytes") and st.session_state.get("daily_file_name"):
        st.download_button(
            "📥 통합 CSV 다운로드",
            data=st.session_state.daily_file_bytes,
            file_name=st.session_state.daily_file_name,
            mime=st.session_state.get("kw_file_mime", "text/csv"),
            key="dl_daily",
        )
...
with col_kw:
    ...
    # ✅ 기간/날짜 위젯 동기화
    _ensure_range_state("kw", default_preset="주간(월~일)")
    ...
    st.selectbox(
        ...
        ["주간(월~일)", "어제", "지난 7일", "지난 30일", "이번 달", "직접선택"],
        key="kw_preset",
        on_change=lambda: _apply_preset("kw"),
    )

    col_k1, col_k2 = st.columns(2)
    with col_k1:
        kw_d1 = st.date_input("시작일", key="kw_d1", on_change=lambda: _mark_custom("kw"))
    with col_k2:
        kw_d2 = st.date_input("종료일", key="kw_d2", on_change=lambda: _mark_custom("kw"))
    ...
    if st.button("키워드 성과 엑셀 생성", type="primary", key="btn_kw"):
        ...
        if kw_fname:
            # run_keyword_report는 (xlsx_path, csv_path)를 반환
            if isinstance(kw_fname, (list, tuple)) and len(kw_fname) >= 2:
                xlsx_path, csv_path = kw_fname[0], kw_fname[1]
            else:
                xlsx_path, csv_path = kw_fname, None

            # ✅ 빠른 다운로드: CSV를 우선 저장 (없으면 XLSX)
            if csv_path and os.path.exists(csv_path):
                with open(csv_path, "rb") as f:
                    st.session_state.kw_file_bytes = f.read()
                st.session_state.kw_file_name = os.path.basename(csv_path)
                st.session_state.kw_file_mime = "text/csv"
                st.success("✅ 키워드 성과 CSV 생성 완료!")
            elif xlsx_path and os.path.exists(xlsx_path):
                with open(xlsx_path, "rb") as f:
                    st.session_state.kw_file_bytes = f.read()
                st.session_state.kw_file_name = os.path.basename(xlsx_path)
                st.session_state.kw_file_mime = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                st.success("✅ 키워드 성과 엑셀 생성 완료!")

# ✅ 생성된 파일이 있으면 언제든 다운로드 가능
    if st.session_state.get("kw_file_bytes") and st.session_state.get("kw_file_name"):
        st.download_button(
            "📥 키워드 성과 다운로드",
            data=st.session_state.kw_file_bytes,
            file_name=st.session_state.kw_file_name,
            mime=st.session_state.get("kw_file_mime", "text/csv"),
            key="dl_kw",
        )
...
with col_comment:
    ...
    # ✅ 코멘트 출력은 항상 렌더 (다른 위젯/챗봇 상호작용으로 rerun 되어도 내용 유지)
    ...
    if st.button("데일리 코멘트 생성", key="btn_comment"):
        if not st.session_state.saved_path:
            st.warning("먼저 통합 리포트를 생성해주세요")
        else:
            with st.spinner("코멘트 생성 중..."):
                try:
                    comment = generate_daily_comment_from_file(
                        st.session_state.saved_path,
                        st.session_state.saved_platform,
                        compare_mode,
                        st.session_state.get("manual_actions", "") or ""
                    )
                except Exception:
                    comment = f"❌ 코멘트 생성 오류:\n{traceback.format_exc()}"
            # ✅ session_state에 저장해서 rerun에도 유지
            st.session_state["comment_out"] = comment

    st.text_area("데일리 코멘트", st.session_state["comment_out"], height=400, key="comment_out")
            ...
            with st.spinner("생각 중..."):
                try:
                    ...
                    if st.session_state.saved_path:
                        try:
                            df_ctx = pd.read_csv(st.session_state.saved_path, encoding="utf-8-sig") if str(st.session_state.saved_path).lower().endswith(".csv") else pd.read_excel(st.session_state.saved_path)
                            if len(df_ctx) > 500:
