...
# =========================================================
# Gemini (lazy init: 키/패키지 없을 때 앱이 죽지 않게)
# =========================================================
GEMINI_API_KEY = _secret("GEMINI_API_KEY")
GEMINI_MODEL   = "gemini-2.5-flash"
_gemini_client = None
def get_gemini_client():
    """Gemini client를 필요할 때만 생성"""
    global _gemini_client
    if _gemini_client is not None:
        return _gemini_client
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY가 설정되어 있지 않습니다 (Streamlit Secrets 또는 환경변수).")
    try:
        from google import genai as _genai
    except Exception as e:
        raise RuntimeError(f"google-genai 패키지를 불러올 수 없습니다: {e}")
...
def _apply_preset(prefix: str):
    preset = st.session_state.get(f"{prefix}_preset", "어제")
    s_iso, e_iso = preset_range(preset)
    st.session_state[f"{prefix}_d1"] = datetime.strptime(s_iso[:10], "%Y-%m-%d").date()
    st.session_state[f"{prefix}_d2"] = datetime.strptime(e_iso[:10], "%Y-%m-%d").date()
...
def run_all(platform, d_f, d_t, tabula_file=None):
    """데일리 통합 리포트 생성 (CSV)
    - CSV는 시트 구조가 없지만 가장 빠르게 다운로드 가능
    """
    try:
        ...
        fname = f"통합리포트_{ts}.csv"
        df.to_csv(fname, index=False, encoding="utf-8-sig")
        logs.append(f"✅ CSV 저장: {fname}")
        logs.append(f"✅ 최종 행수 = {len(df)}")
        # ✅ 키워드 캐시 저장 (코멘트/챗봇에서 재사용)
        try:
            kw_cache = {}
            if "Google" in platform:
            ...
            kw_cache_path = f"{base}_kw_cache.json"
...
def run_keyword_report(platform, d1, d2):
    try:
        ...
        fname = f"키워드성과_가공본_{datetime.now().strftime('%m%d_%H%M')}.csv"
...
def _build_conv_keywords_map(platform: str, d: pd.Timestamp, top_n=5, logs=None, excel_path: str = None):
    ...
    if excel_path:
        ...
        cache_path = f"{base}_kw_cache.json"
...
""".strip()
    ...
    resp = get_gemini_client().models.generate_content(model=GEMINI_MODEL, contents=full_prompt)
...
st.markdown("""
<style>
.hero { background: linear-gradient(135deg,#2563eb,#60a5fa); border-radius:16px;
...
with col_daily:
    ...
    if "daily_d1" not in st.session_state or "daily_d2" not in st.session_state:
        _apply_preset("daily")
...
with col_kw:
    ...
    if "kw_d1" not in st.session_state or "kw_d2" not in st.session_state:
        _apply_preset("kw")
    ...
    if st.button("키워드 성과 엑셀 생성", type="primary", key="btn_kw"):
        ...
        if kw_fname and os.path.exists(kw_fname):
            ...
            st.session_state.kw_file_name = os.path.basename(kw_fname)

    if st.session_state.get("kw_file_bytes"):
        st.download_button(
            "📥 키워드 성과 CSV 다운로드",
...
with col_comment:
    ...
    st.text_area("데일리 코멘트", key="comment_out", height=400)
            ...
            with st.spinner("생각 중..."):
                try:
                    ...
                    if st.session_state.saved_path:
                        try:
                            ext = os.path.splitext(st.session_state.saved_path)[1].lower()
                            if ext == ".csv":
                                df_ctx = pd.read_csv(st.session_state.saved_path)
                            else:
                    ...
                    resp = get_gemini_client().models.generate_content(model=GEMINI_MODEL, contents=gemini_history)
